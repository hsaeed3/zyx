...
def _prepare_make_request(
    ...
    agent = core.prepare_agent(
        ...
        deps_type = type(params.get("deps", None)) if params.get("deps", None) else None,
    )
    ...
    parsed_tools = core.prepare_tools(
        params.get("tools", None)
    )
...
def make(
    ...
    **kwargs : Any,
    ...
    """
    Generate ('make') a result in a `target` type using an LLM or Agent from
    `pydantic_ai`.
    ...
    ----------
    context : ContextParam | List[ContextParam]
        The context to use for the semantic operation.
    target : TargetParam = str
        The target type to generate a result for.
    model : ModelParam = "openai:gpt-4o-mini"
        The model to use for the semantic operation.
    model_settings : ModelSettings | None = None
        The model settings to use for the semantic operation.
    instructions : InstructionsParam | None = None
        The instructions to use for the semantic operation.
    tools : ToolParam | List[ToolParam] | None = None
        The tools to use for the semantic operation.
    deps : DepsType | None = None
        The dependencies to use for the semantic operation.
    usage_limits : UsageLimits | None = None
        The usage limits to use for the semantic operation.
    **kwargs : Any
        Additional keyword arguments to pass to the semantic operation.
    ...
    AgentRunError
        If an error occurs while running the agent.

    Returns
    -------
    Result[Output]
        A `Result` object containing the output of the semantic operation.
    """
    params = {
        "context" : context,
        "target" : target,
    ...
    agent, request_params = _prepare_make_request(
        params = params
    )
    ...
    return Result(
        ...
        raw = result,
        models = [agent.model if isinstance(agent.model, str) else agent.model.model_name] # type: ignore
    )
...
async def amake(
    ...
    **kwargs : Any,
    ...
    """
    Asynchronously generate ('make') a result in a `target` type using an LLM or Agent from
    `pydantic_ai`.
