...
from ...processing.schemas.semantics import (
...
def _generate_mock_value(
    field_type: Type[Any], field_name: str | None = None
) -> Any:
    """Generate a mock value for a given type.

    Uses semantic utilities to generate more contextually appropriate mock values.

    Parameters
    ----------
    field_type : Type[Any]
        The type to generate a mock value for.
    field_name : str | None
        Optional field name for context-aware value generation.

    Returns
    -------
    Any
        A mock value appropriate for the given type.

    Examples
    --------
        >>> _generate_mock_value(int)
        42

        >>> _generate_mock_value(str, "name")
        'mock_name'

        >>> _generate_mock_value(list[str])
        ['mock_item']
    """
    ...
    if origin is type(lambda: None) or str(origin) == "typing.Union":
        ...
        for arg in args:
            if arg is not type(None):
                return _generate_mock_value(arg, field_name)
        return None
    ...
    # Handle basic types with semantic field names
    if field_type is str or field_type == str:
        # Use field name if provided for more semantic mock values
        if field_name:
            return f"mock_{field_name}"
        # Otherwise use semantic key
        try:
            semantic_key = to_semantic_key(field_type)
            return f"mock_{semantic_key}"
        except:
            return "mock_string"
    ...
    if isinstance(field_type, type) and issubclass(field_type, BaseModel):
        ...
        for field_name, field_info in field_type.model_fields.items():
            mock_data[field_name] = _generate_mock_value(
                field_info.annotation, field_name
            )
        return field_type(**mock_data)

    # Default fallback - try to use semantic key
    try:
        semantic_key = to_semantic_key(field_type)
        return f"mock_{semantic_key}"
    except:
...
def _create_mock_tool_call(
    tool: dict, call_id: str
) -> ChatCompletionMessageToolCall:
    """Create a mock tool call response.

    Generates semantically appropriate mock arguments based on the tool's
    parameter schema, using semantic utilities for better context.

    Parameters
    ----------
    tool : dict
        The tool definition with function name and parameters schema.
    call_id : str
    ...
    -------
    ChatCompletionMessageToolCall
        A mock tool call with generated arguments.
    ...
    --------
        ...
        ...     }
        ... }
        >>> call = _create_mock_tool_call(tool, "call_1")
        >>> call.function.name
        'search'
        >>> args = json.loads(call.function.arguments)
        >>> 'query' in args and 'limit' in args
        True
    """
    ...
    if "properties" in parameters:
        for prop_name, prop_schema in parameters["properties"].items():
            ...
            # Check for enum values first (Literal types)
            if "enum" in prop_schema:
                mock_arguments[prop_name] = prop_schema["enum"][0]
            elif prop_type == "string":
                # Use semantic naming for better mock values
                ...
                # Generate an array with one mock item if item type is specified
                if "items" in prop_schema:
                    item_type = prop_schema["items"].get("type", "string")
                    if item_type == "string":
                        mock_arguments[prop_name] = [
                            f"mock_{prop_name}_item"
                        ]
                    elif item_type == "integer":
                        mock_arguments[prop_name] = [1]
                    elif item_type == "number":
                        mock_arguments[prop_name] = [1.0]
                    else:
                        mock_arguments[prop_name] = []
                else:
                    mock_arguments[prop_name] = []
        ...
        **kwargs: Any,
        ...
        if tools:
            ...
            if parallel_tool_calls is False or parallel_tool_calls is None:
                ...
                tool_calls.append(
                    _create_mock_tool_call(tools[0], f"call_mock_1")
                )
            else:
                # Call all tools
                for idx, tool in enumerate(tools):
                    tool_calls.append(
                        _create_mock_tool_call(
                            tool, f"call_mock_{idx + 1}"
                        )
                    )
...
class MockModelClient(ModelClient):
    ...
    async def batch_chat_completion(
        ...
        **kwargs: Any,
        ...
        """Generate multiple mock chat completions concurrently."""
        return await self._openai_client.batch_chat_completion(
            model=model,
            messages=messages,
            **kwargs,
        )
    ...
    async def batch_structured_output(
        ...
        **kwargs: Any,
        ...
        """Generate multiple mock structured outputs concurrently."""
        return await self._openai_client.batch_structured_output(
            model=model,
            messages=messages,
            response_model=response_model,
            instructor_mode=instructor_mode,
