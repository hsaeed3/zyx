...
from ..._internal._exceptions import ModelDefinitionError
...
class ModelSettings(ABC):
    ...
    def kind(self) -> str:
        ...
        raise NotImplementedError(
            "Subclasses must implement the 'kind' property"
        )

    def __str__(self):
        from ..._internal._beautification import Beautifier

        return Beautifier.for_settings(
            type_name=self.kind,
...
class ModelDefinition(ABC):
        ...
        *,
        ...
        if base_url:
            ...
            if ModelProviderRegistry().get(f"custom:{base_url}"):
                self._provider = ModelProviderRegistry().get(
                    f"custom:{base_url}"
                )
            else:
    ...
    def get_client(self) -> ModelClient:
        ...
        if not self._client:
            ...
            if (
                self._model.lower() == "mock"
                or self._model.lower().startswith("mock/")
            ):
    ...
    def __str__(self):
        ...
        return Beautifier.for_model(
            ...
            main_params=self.__dict__,
            metadata=self.provider.__dict__,
            nested=self.settings,
        ).__str__()
