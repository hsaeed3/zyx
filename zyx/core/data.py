...
def embeddings(
    ...
    if isinstance(text, list):
        ...
        if model.startswith("openai/") or model.startswith("text-embedding"):
            ...
            if verbose:
                print(f"Initialized [bold green]{model}[/bold green] through the [bold blue]OpenAI[/bold blue] Client.")
            ...
            if verbose:
                print(f"Initialized [bold green]{model}[/bold green] through the [bold blue]Ollama[/bold blue] Client.")
            return embedder.get_embedding(text)
            ...
            if verbose:
                print(f"Initialized [bold green]{model}[/bold green] through the [bold blue]Mistral[/bold blue] Client.")
            return embedder.get_embedding(text)
            ...
            if verbose:
                print(f"Initialized [bold green]{model}[/bold green] through the [bold blue]Google[/bold blue] Client.")
            return embedder.get_embedding(text)
...
class Database:
    ...
    def search(self, query: str, max_results: Optional[int] = 5):
        ...
        text_results = Document.select().where(Document.text.contains(query)).limit(max_results)
    ...
    def update_document(self, doc_id: str, text: Optional[str] = None, metadata: Optional[dict] = None):
        from qdrant_client.http import models as rest_models
        update_data = {}
        if text is not None:
        ...
        if update_data:
            updated_rows = Document.update(update_data).where(Document.id == doc_id).execute()
            if updated_rows == 0:
                return False
        ...
        if text is not None or metadata is not None:
            ...
            if current_doc:
                ...
                new_metadata = metadata if metadata is not None else eval(current_doc.metadata)
