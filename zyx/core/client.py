...
from .ext import BaseModel, zyxModuleLoader
...
class zyxLanguageClient:
    def __init__(
        ...
        if self.params.verbose:
            print(f"Connected to [bold green]{self.params.model}[/bold green] through the [bold blue]{self.params.provider}[/bold blue] client.")
    ...
    def _connect_llm(
        ...
        elif self.params.model.startswith("anthropic/") \
            or self.params.model.startswith("claude-"):
        ...
        elif self.params.model.startswith("ollama/") \
            or self.params.model.startswith("ollama_chat/"):
...
def function(
    ...
    **kwargs
    ...
    def decorator(f: Callable) -> Callable:
        @wraps(f)
        @tenacity.retry(stop=tenacity.stop_after_attempt(3), retry=tenacity.retry_if_exception_type())
        def function_wrapper(*args, **kwargs):
            from pydantic import create_model
            type_hints = get_type_hints(f)
            return_type = type_hints.pop('return', Any)
            FunctionResponseModel = create_model('FunctionResponseModel',
                input=(dict[str, Any], ...),
                output=(return_type, ...)
            )
...
def classify(
    ...
    ResponseModel = create_model('ResponseModel', items=(List[ClassificationResult], ...))
    ...
    if isinstance(inputs, str):
        inputs = [inputs]
    user_message = f"Classify the following text(s):\n\n" + "\n\n".join(inputs)
