"""
zyx.core.chat_completions.helpers.tool_calling
...
def convert_to_openai_tool(tool: Union[Callable, BaseModel, Dict[str, Any]]):
    ...
    if isinstance(tool, Dict):
        if "type" in tool and tool["type"] == "function":
            ...
            raise utils.ZyxException(f"Tool {utils.Styles.module(tool)} is not an OpenAI tool dictionary.")
    ...
    if callable(tool):
        try:
            ...
            raise utils.ZyxException(
                f"Failed to convert Python function {utils.Styles.module(tool)} to OpenAI tool: {e}"
            )
    ...
    if isinstance(tool, BaseModel):
        ...
        try:
            ...
            raise utils.ZyxException(
                f"Failed to convert Pydantic model {utils.Styles.module(tool)} to OpenAI tool: {e}"
            )
