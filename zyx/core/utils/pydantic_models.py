...
def convert_to_pydantic_model_cls(
    ...
    if isinstance(target, str):
        ...
        if logging.zyx_debug:
            logging.debug(f"Converted string {target} to Pydantic model class: {model.model_json_schema()}")
    ...
    if isinstance(target, type) and issubclass(target, BaseModel):
        ...
        if logging.zyx_debug:
            logging.debug(f"Returning class type for initialized Pydantic model: {target.model_json_schema()}")
        ...
        if logging.zyx_debug:
            logging.debug(f"Converted generic type {target} to Pydantic model class: {model.model_json_schema()}")
    ...
    # Handle list/sequence of fields
        ...
        if logging.zyx_debug:
            logging.debug(f"Converted {target} to Pydantic model class: {model.model_json_schema()}")
    ...
    # Handle dict specification
        ...
        if logging.zyx_debug:
            logging.debug(f"Converted dictionary {target} to Pydantic model class: {model.model_json_schema()}")
