"""
zyx.core.logging.chat_messages
...
def create_chat_message(
    ...
    if images:
        ...
        if logging.zyx_debug:
            logging.logger.debug(f"Processed images for {role} message: {processed_images}")
...
def convert_to_chat_messages(
    ...
    if isinstance(messages, str):
        if logging.zyx_debug:
            logging.logger.debug(f"Formatting single string into message thread: [italic dim]{messages}[/italic dim]")
    ...
    if logging.zyx_debug:
        logging.logger.debug(f"Formatted {len(messages)} messages into message thread.")
...
def format_or_create_system_chat_message(
    ...
    if len(system_messages) > 1:
        ...
        if not messages[0] == system_messages[0]:
            ...
            if logging.zyx_debug:
                logging.logger.debug(f"Validated system message position in thread.")
...
def add_system_context_to_thread(
    ...
    if any(message.role == "system" for message in messages):
        ...
        if logging.zyx_debug:
            logging.logger.debug(f"Added system context to thread as a new system message.")
    # Return
    return messages
...
def add_user_context_to_thread(
    ...
    if messages and messages[-1].role == "user":
        ...
        if logging.zyx_debug:
            logging.logger.debug(f"Added user context to thread as a new user message.")
