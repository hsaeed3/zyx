...
def prettify_result(result : Result) -> RenderableType:
    ...
    renderables.append(
        Text.from_markup(f"[sandy_brown]>>>[/sandy_brown] [dim italic]Operation: {result.kind.capitalize()}[/dim italic]")
    )

    if result.models:
        model_names = result.models[0] if len(result.models) == 1 else ", ".join(result.models)
        label = "Model" if len(result.models) == 1 else "Models"
        model_info = Text.from_markup(f"[sandy_brown]>>>[/sandy_brown] [dim italic]{label}: {model_names}[/dim italic]")
