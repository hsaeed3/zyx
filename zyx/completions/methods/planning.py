...
def planner(
    target: Union[str, Type[BaseModel]],
    ...
    if isinstance(target, str):
        system_message = _get_string_system_message(target, steps)
        response_model = Plan
    elif isinstance(target, type) and issubclass(target, BaseModel):
        system_message = _get_model_system_message(target, steps)
        response_model = create_model(f"{target.__name__}Plan", tasks=(List[target], ...))
    else:
        raise ValueError("Target must be either a string or a Pydantic model class.")
