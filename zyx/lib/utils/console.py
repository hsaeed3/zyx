...
# console helper
def get_zyx_console() -> zyxConsole:
    return zyxConsole(
        theme=Theme({
            "info": "dim cyan",
            "warning": "dim yellow",
...
def get_console_instance() -> zyxConsole:
    global console_instance
    with console_lock:
        if console_instance is None:
            console_instance = get_zyx_console()
...
# logger helper
def get_zyx_logger(console: Optional[zyxConsole] = None) -> logging.Logger:
    logger = logging.getLogger(__name__)
    logger.setLevel(logging.INFO)
    
    # Use provided console or create a new one if not provided
    if console is None:
        console = get_zyx_console()
...
def get_logger_instance() -> logging.Logger:
    ...
    with logger_lock:
        if logger_instance is None:
            logger_instance = get_zyx_logger()
