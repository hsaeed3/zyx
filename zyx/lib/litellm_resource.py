        ...
        if not hasattr(cls, '_instance'):
            try:
                ...
                if cls.litellm is None:
                    ...
                    if _utils.zyx_debug:
                        _utils.logger.debug(
                            f"Created {_utils.Styles.module('LiteLLM')} resource singleton successfully."
                        )
                ...
                _utils.ZyxException(f"Failed to initialize LiteLLM resource: {e}")
        else:
            if _utils.zyx_debug:
                _utils.logger.debug(
                    f"Loaded prexisting {_utils.Styles.module('LiteLLM')} resource successfully."
                )
