...
class Client:
    ...
    def __init_client__(self) -> clienttypes.ClientProviders:
        ...
        return clienttypes.ClientProviders(client=client, instructor=patch)
    ...
    @staticmethod
    def _get_tool_dict(tools: List[clienttypes.Tool]) -> List[Dict[str, Any]]:
    ...
    def _format_to_openai_tools(
        self, tools: List[clienttypes.ToolType]
    ) -> List[clienttypes.Tool]:
        """Converts the tools to a list of dictionaries.
    ...
    def _chat_completion(
        self, args: clienttypes.CompletionArgs
        ...
        try:
            ...
            if args.model.startswith("o1-"):
                logger.warning(
                    "Removing all unsupported parameters for OpenAI o1 or o1-mini"
                )
            ...
            if args.stream:
                ...
                response = self.providers.client.chat.completions.create(
                    **args.model_dump(exclude=exclude_params), stream=False
                )
                return response

        except TypeError as e:
            logger.error(
                f"Serialization error in base chat completion: {e}. \n Args: {args}"
            )
            raise e
        except Exception as e:
    ...
    def _instructor_chat_completion(self, args: clienttypes.CompletionArgs) -> Any:
    ...
    def completion(
        ...
        if self.config.verbose:
            logger.info(
                f"Running completion with {len(messages)} messages, with {model}."
            )
...
def completion(
    ...
    if client_mode == "openai":
        client_mode, model, base_url, api_key = Client._recommend_client_by_model(
            model, base_url
        )
        if client_mode == "litellm":
            logger.info("Defaulting to LiteLLM client.")
