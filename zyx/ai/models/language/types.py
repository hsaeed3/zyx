...
def _rich_pretty_print_language_model_response(response: LanguageModelResponse):
    ...
    if response.provider is not None:
        output_table.add_row(
            "[dim sandy_brown]Provider:[/dim sandy_brown]",
            f"[dim italic]{escape(response.provider)}[/dim italic]",
        )
...
class LanguageModelResponse(BaseModel, Generic[T]):
    ...
    """The model provider name associated with this response, if known."""
