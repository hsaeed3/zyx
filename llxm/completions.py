...
class Completions:
    ...
    def __init__(
        ...
        try:
            ...
            raise LLXMException(f"Failed to initialize litellm methods: {e}")
    ...
    # instructor patch
    def instructor_patch(self, mode : CompletionsInstructorMode = "tool_call") -> None:
        """
    ...
    def instructor_apatch(self, mode : CompletionsInstructorMode = "tool_call") -> None:
        """
    ...
    def _run_completion(
        ...
        if args.response_model is not None:
            ...
            try:
                ...
                raise LLXMException(f"Failed to patch instructor: {e}")
            ...
            try:
                ...
                raise LLXMException(f"Failed to run instructor completion: {e}")
            ...
            try:
                ...
                raise LLXMException(f"Failed to run completion: {e}")
    ...
    def run_completion(
        ...
        try:
            ...
            raise LLXMException(f"Failed to validate messages, please ensure they are formatted correctly: {e}")
        
        # build args
        try:
            args = utils.collect_completion_args(locals())
        except Exception as e:
            raise LLXMException(f"Failed to build completion arguments: {e}")
                ...
                for tool in tools:
                    if isinstance(tool, str):
                        try:
                            ...
                            raise LLXMException(f"Failed to generate tool named: {tool}: {e}")
                    else:
                        args.tools.append(tool_calling.convert_to_tool(tool))
            except Exception as e:
                raise LLXMException(f"Failed to format tools: {e}")
        ...
        if is_batch_completion:
            try:
                ...
                raise LLXMException(f"Failed to run batch completion: {e}")
        ...
        if tools and run_tools is True:
                ...
                for tool_call in response.choices[0].message.tool_calls:
                    ...
                    if tool_call.function.name in [tool.name for tool in args.tools]:
                        ...
                        try:
                            ...
                            raise LLXMException(f"Failed to run tool: {e} with args: {tool_call.function.arguments}")
            ...
            try:
                ...
                raise LLXMException(f"Failed to run final completion: {e}")
