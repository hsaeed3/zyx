...
from .types.completions.completions_tool import CompletionsTool
from .exceptions import LLXMException


# tool generator
def generate_tool(client, tool_name: str, model: str) -> CompletionsTool:
...
def convert_to_tool(
    ...
    # setup tool
    tool = CompletionsTool(function = function)

    try:
        # format tool
        tool.formatted_function = convert_to_openai_tool(function)
    except Exception as e:
        raise LLXMException(f"Failed to convert tool: {e}")
    ...
    try:
        ...
        raise LLXMException(f"Failed to format tool: {e}")
