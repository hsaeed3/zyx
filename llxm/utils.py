...
def parse_json_string_to_pydantic_model(json_string: str, response_format: Optional[BaseModel] = None) -> BaseModel:
    ...
    try:
        ...
        raise LLXMException(f"Invalid JSON string: {e}")
    except ValidationError as e:
        raise LLXMException(f"Validation error: {e}")
    except Exception as e:
        raise LLXMException(f"Failed to parse JSON string to pydantic model: {e}")
...
def collect_completion_args(args : dict[str, Any]) -> CompletionsArguments:
    ...
    try:
        ...
        raise LLXMException(f"Validation error while building completion arguments: {e}")
    except Exception as e:
